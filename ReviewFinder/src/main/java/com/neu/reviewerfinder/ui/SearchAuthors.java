...
public class SearchAuthors extends Application {
   ...
   * @param primaryStage
   */
  private void advancedSearch(ArrayList<String> list, String titleStr, LocalDate dateSince, String minPaper,
      Integer consecutiveYear, String authorName, ProgressIndicator loadingIndicator,
      Stage primaryStage) {
    if (loadingIndicator.isVisible()) {
      return;
    }
    ...
    Task<Boolean> advancedSearch = new Task<Boolean>() {
      {
        setOnSucceeded(workerStateEvent -> {
          if (getValue()) {
            ...
            SearchError sa =
                new SearchError("Please enter valid input/ Select atleast one criteria",2);
            sa.start(primaryStage);
          }
        });
  ...
  public void start(Stage primaryStage) {
    ...
    btn1.setOnAction(event -> advancedSearch(list, titleText.getText(),
        datePicker.getValue(), minPaper.getText(), spinner.getValue(), authorName.getText(),
        loadingIndicator, primaryStage));
